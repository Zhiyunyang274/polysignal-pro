...
## ADR-030：策略转向——从方向性交易到做市模式

### 状态

Accepted (2026-09-13，用户批准方向调整)

### 背景

28 轮迭代、23 笔真实前向仓位、4 种 edge 类型的全部证据指向一个结论：
**当前 Polymarket 市场上可被批量扫描检测到的方向性定价偏差，全部在交易成本
与风险之下**。crypto_price_threshold_v1 降级 quarantined（16 closed / 6.2%
胜率 / 全负）；stale_price_lag 不可测试（无近屏障市场）；概率系列隔离。

与此同时，Polymarket 上持续盈利的参与者使用的核心策略是**做市**——
双边报价赚取买卖价差，方向风险通过库存管理控制。这与方向性交易的风险收益
结构根本不同：做市不需要预测对错，只需要定价准确、报价合理、风控到位。

### 决策

策略方向从**方向性交易**转向**做市模式**：

1. 新增 `MarketMaker` 模块：围绕公允价值双边报价，库存偏斜，独立于方向性信号
2. 复用现有基础设施：AccountState（账本）、Risk Governor（风控）、SimBroker（撮合）
3. 分阶段推进：先模拟盘验证做市行为 → 再接入实时数据 → 最后小额真实测试
4. 方向性模块保留但不再新增投入（crypto_price_threshold_v1 已 quarantined）

### 影响

优点：消除对方向性 alpha 的依赖；价差收入是结构性的而非预测性的；
现有风控/账本/测试基础设施直接可用。
代价：做市需要不同的定价模型（报价而非预测）；库存风险需要新管理方式；
竞争环境下价差可能被压缩。
维持不变：全部安全约束（live=false、风控门、fail-closed 证据处理）。
